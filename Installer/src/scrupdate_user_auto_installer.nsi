...
!macro InitializeGlobalVariables
	...
	Name "${PROGRAM_NAME}"
	OutFile "..\bin\tmp\${USER_AUTO_INSTALLER_NAME}"
...
Section
	...
	${Else}
		...
		${Else}
			...
			WriteRegStr HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\${PROGRAM_NAME}.exe" "" "$INSTDIR\${PROGRAM_NAME}.exe"
			WriteRegStr HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\${PROGRAM_NAME}.exe" "Path" "$INSTDIR"
			WriteRegStr HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PROGRAM_NAME}" "DisplayName" "${PROGRAM_NAME}"
			WriteRegStr HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PROGRAM_NAME}" "DisplayIcon" "$INSTDIR\${PROGRAM_NAME}.exe"
			WriteRegStr HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PROGRAM_NAME}" "DisplayVersion" "${PROGRAM_VERSION}"
			WriteRegStr HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PROGRAM_NAME}" "Publisher" "${PROGRAM_PUBLISHER}"
			WriteRegStr HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PROGRAM_NAME}" "UninstallString" "$INSTDIR\${UNINSTALLER_NAME}"
...
Section "uninstall"
	...
	${Else}
		...
		${Else}
			...
			${EndIf}
			DeleteRegKey HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\${PROGRAM_NAME}.exe"
			DeleteRegKey HKCU "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${PROGRAM_NAME}"
			${If} ${RunningX64}
